...
model = load_model('IMDN@Base')  #model select

window_size = 16  # Define windoes_size of D
img_lr, img_hr = prepare_images('/opt/data/private/HSJ/LAM_Demo-main/test_images/7.png')  # Change this image name
...
interpolated_grad_numpy, result_numpy, interpolated_numpy = Path_gradient(tensor_lr.cpu().numpy(), model, attr_objective, gaus_blur_path_func)
...
pil.show()
path = '/opt/data/private/HSJ/HFEN-main/LAM/LAM_Demo-main/7.png'  # 保存地址
try:
    pil.save(path)
